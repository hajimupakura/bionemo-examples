...
# AI-Powered Analysis Configuration (hidden from users)
AI_API_KEY = "sk-cb3e2fbf705d4c1899a7cf53c49fbaa6"  # Replace with your actual key
ai_client = OpenAI(api_key=AI_API_KEY, base_url="https://api.deepseek.com")  # Hidden backend detail

# Prompt Templates for AI-Powered Analysis
SYSTEM_PROMPT = """
You are an advanced AI assistant fine-tuned for computational drug discovery analysis. Your role is to analyze provided data, summarize findings, draw conclusions, and offer actionable recommendations based on scientific principles and computational chemistry best practices. Structure your response as:
- **Summary**: Key findings from the data
- **Conclusions**: Insights and interpretations
- **Recommendations**: Practical next steps or adjustments
- **Questions?**: "Ask me to elaborate on any point or explore further!"
Use the data provided and standard drug discovery frameworks (e.g., Lipinski rules, pharmacophore design, SAR methodologies). Highlight uncertainties or assumptions where applicable.
"""

TARGET_PROMPT = """
I’ve analyzed a protein structure with the following details:
- Protein: {current_protein}
- Pockets: {pockets_data}
- Visualization Settings: {viz_settings}

Summarize the pocket analysis, conclude on the protein's druggability and potential binding sites, and recommend steps for pharmacophore modeling or screening.
"""

PHARMA_PROMPT = """
I’ve generated a pharmacophore model with these details:
- Model ID: {model_id}
- Features: {features_data}
- Method: {pharma_method}
- Pocket: {selected_pocket}
- Reference Ligand: {ref_ligand_info}

Summarize the features, conclude on the model’s suitability for virtual screening, and recommend refinements or screening strategies.
"""

SCREENING_PROMPT = """
I’ve screened compounds with these results:
- Model: {selected_model}
- Hits: {results_data}
- Filters: {selected_filters}
- Parameters: {screening_params}

Summarize the outcomes, conclude on hit quality and drug-likeness, and recommend next steps (e.g., SAR, synthesis).
"""

SAR_PROMPT = """
I’ve performed SAR analysis with:
- Screen ID: {selected_screen}
- Results: {sar_results}
- Activity Source: {activity_source}
- Analyses Run: {analysis_type}

Summarize the findings, conclude on structural features driving activity, and recommend optimization strategies.
"""

ANALYZE_ALL_PROMPT = """
I’ve completed a drug discovery pipeline with:
- Proteins: {analyzed_proteins_summary}
- Pharmacophores: {pharmacophores_summary}
- Screening Results: {screened_compounds_summary}
- SAR Analysis: {sar_summary}

Summarize key findings across all stages, conclude on project progress and target viability, and recommend a cohesive strategy moving forward.
"""
...
Provide a detailed, scientifically grounded response, building on the prior analysis and provided data.
"""
# Add the new AI-Powered Analysis Tab
with tab6:
    st.header("AI-Powered Analysis")
    st.write("Leverage an advanced, fine-tuned AI assistant to analyze your drug discovery pipeline and gain expert insights.")
    ...
    # Analysis selection
    analysis_options = ["Target Characterization", "Pharmacophore Modeling", 
                       "Virtual Screening", "SAR Analysis", "Analyze All"]
    selected_analysis = st.multiselect(
        "Select analysis scope",
        analysis_options,
        default=["Analyze All"],
        help="Choose one or more stages, or 'Analyze All' for a comprehensive pipeline review."
    )
    ...
    if st.button("Run AI Analysis"):
        ...
        elif not any([st.session_state.analyzed_proteins, st.session_state.pharmacophores, 
                      st.session_state.screened_compounds]):
            st.warning("No data available. Complete at least one analysis stage first.")
        else:
            with st.spinner("Running AI-powered analysis..."):
                # Prepare context
                context = {}
                if "Target Characterization" in selected_analysis or "Analyze All" in selected_analysis:
                    context["current_protein"] = st.session_state.current_protein
                    context["pockets_data"] = st.session_state.analyzed_proteins.get(
                        st.session_state.current_protein, {}).get("pockets", "No pockets analyzed")
                    context["viz_settings"] = visualization_quality
                if "Pharmacophore Modeling" in selected_analysis or "Analyze All" in selected_analysis:
                    latest_pharma = max(st.session_state.pharmacophores.items(), 
                                      key=lambda x: x[1]["timestamp"], default=(None, None))[0]
                    context["model_id"] = latest_pharma or "None"
                    context["features_data"] = st.session_state.pharmacophores.get(
                        latest_pharma, {}).get("model", {}).get("features", "No features")
                    context["pharma_method"] = st.session_state.pharmacophores.get(
                        latest_pharma, {}).get("model", {}).get("method", "Unknown")
                    context["selected_pocket"] = "Latest pocket"  # Simplified
                    context["ref_ligand_info"] = "Not provided"  # Add if tracked
                if "Virtual Screening" in selected_analysis or "Analyze All" in selected_analysis:
                    latest_screen = max(st.session_state.screened_compounds.items(), 
                                      key=lambda x: x[1]["timestamp"], default=(None, None))[0]
                    context["selected_model"] = st.session_state.screened_compounds.get(
                        latest_screen, {}).get("model", "None")
                    context["results_data"] = [
                        f"{r['name']}: {r['match_score']}% (MW: {r['descriptors']['MW']:.1f})"
                        for r in st.session_state.screened_compounds.get(latest_screen, {}).get("results", [])[:5]
                    ]
                    context["selected_filters"] = druglikeness_filter
                    context["screening_params"] = "Feature matching: 3"  # Simplified
                if "SAR Analysis" in selected_analysis or "Analyze All" in selected_analysis:
                    latest_screen = max(st.session_state.screened_compounds.items(), 
                                      key=lambda x: x[1]["timestamp"], default=(None, None))[0]
                    results = st.session_state.screened_compounds.get(latest_screen, {}).get("results", [])
                    context["selected_screen"] = latest_screen or "None"
                    context["sar_results"] = f"Active: {sum(1 for r in results if r.get('active', False))}, " \
                                           f"Inactive: {len(results) - sum(1 for r in results if r.get('active', False))}"
                    context["activity_source"] = "Match Score"  # Placeholder
                    context["analysis_type"] = "Scaffold, Property"  # Placeholder
                if "Analyze All" in selected_analysis:
                    context["analyzed_proteins_summary"] = f"{len(st.session_state.analyzed_proteins)} proteins"
                    context["pharmacophores_summary"] = f"{len(st.session_state.pharmacophores)} models"
                    context["screened_compounds_summary"] = f"{len(st.session_state.screened_compounds)} runs, " \
                                                          f"{sum(len(v['results']) for v in st.session_state.screened_compounds.values())} hits"
                    context["sar_summary"] = "Available" if any(r.get("active") is not None for v in st.session_state.screened_compounds.values() for r in v["results"]) else "Not performed"

                # Select and format prompt
                if "Analyze All" in selected_analysis and len(selected_analysis) == 1:
                    prompt = ANALYZE_ALL_PROMPT.format(**context)
                else:
                    prompt_parts = []
                    if "Target Characterization" in selected_analysis:
                        prompt_parts.append(TARGET_PROMPT.format(**context))
                    if "Pharmacophore Modeling" in selected_analysis:
                        prompt_parts.append(PHARMA_PROMPT.format(**context))
                    if "Virtual Screening" in selected_analysis:
                        prompt_parts.append(SCREENING_PROMPT.format(**context))
                    if "SAR Analysis" in selected_analysis:
                        prompt_parts.append(SAR_PROMPT.format(**context))
                    prompt = "\n\n".join(prompt_parts)

                # AI call (hidden implementation detail)
                try:
                    response = ai_client.chat.completions.create(
                        model="deepseek-reasoner",  # Backend detail, not exposed to users
                        messages=[
                    ...
                    st.session_state.ai_conversation.append({"role": "assistant", "content": result})
                    st.markdown("### Analysis Output")
                    st.write(result)
                    log_action("AI_Analysis", f"Performed analysis on {', '.join(selected_analysis)}")
                except Exception as e:
    ...
    if st.session_state.ai_conversation:
        ...
        for msg in st.session_state.ai_conversation[-3:]:  # Show last 3 messages
            st.write(f"**{msg['role'].capitalize()}**: {msg['content']}")

        # Follow-up input
        follow_up = st.text_input("Ask a follow-up question:", key="ai_follow_up")
        if st.button("Submit Follow-Up"):
            if not follow_up:
                st.warning("Please enter a follow-up question.")
            else:
                with st.spinner("Processing follow-up..."):
                    previous_output = st.session_state.ai_conversation[-1]["content"]
                    prompt = FOLLOWUP_PROMPT.format(previous_output=previous_output, user_question=follow_up)
                    try:
                        response = ai_client.chat.completions.create(
                            model="deepseek-reasoner",  # Hidden detail
                            messages=[
                                {"role": "system", "content": SYSTEM_PROMPT},
                                {"role": "user", "content": prompt}
                            ],
                        ...
                        result = response.choices[0].message.content
                        st.session_state.ai_conversation.append({"role": "user", "content": follow_up})
                        st.session_state.ai_conversation.append({"role": "assistant", "content": result})
                        st.write("#### Follow-Up Response")
                        st.write(result)
                        log_action("AI_Followup", f"Follow-up question: {follow_up}")
                    except Exception as e:
                        st.error(f"Follow-up analysis failed: {str(e)}")
                        logger.error(f"AI follow-up failed: {str(e)}", exc_info=True)
